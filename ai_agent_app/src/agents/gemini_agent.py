"""
Gemini AI agent with function calling for office automation.
"""
import json
import re
...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            "Create a new Excel (.xlsx) file with a formatted table. "
            "Pass rows as a JSON string in rows_json."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "rows_json": {
                    "type": "string",
                    "description": "Rows as JSON string, e.g. [[\"Alice\",100],[\"Bob\",200]]",
                },
            },
            "required": ["filename"],
        },
    ...
    },
    {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "filename": {"type": "string", "description": "Workbook path, or __ACTIVE__ to use the currently open workbook"},
    ...
    },
    {
        ...
        "description": (
            "Write values into an open Excel workbook starting at a cell. "
            "Pass values as a JSON string in values_json."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "values_json": {
                    "type": "string",
                    "description": "2D JSON array string like [[\"Name\",\"Score\"],[\"Aman\",95]]",
                },
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "new_content": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "New bullet points list",
                },
                "title_font_size": {"type": "integer", "description": "Slide title font size in points"},
                "content_font_size": {"type": "integer", "description": "Slide body font size in points"},
                "title_font_color": {"type": "string", "description": "Slide title font color as hex"},
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "rows": {
                    ...
                    "description": "Table rows as a 2D array",
                },
                "title_font_size": {"type": "integer", "description": "Slide title font size in points"},
...
class GeminiAgent:
    """Core AI agent for office automation with iterative function calling."""
    ...
    def __init__(self, api_key: str, tool_executor: Callable, log_callback: Callable = None):
        ...
        self.model = genai.GenerativeModel(
            ...
            system_instruction=(
                ...
                "When the user asks to create an Excel file, make an Excel sheet, prepare an Excel report, "
                "or otherwise do Excel work, default to the live Excel desktop workflow instead of only file-based creation. "
            ...
            declarations.append(
                genai.protos.FunctionDeclaration(
                    ...
                    parameters=genai.protos.Schema(
                        ...
                        properties={
                            key: self._prop_to_schema(value)
    ...
    def _prop_to_schema(self, prop: dict):
        type_map = {
    ...
    def _is_notepad_request(self, user_message: str) -> bool:
        return "notepad" in user_message.strip().lower()
    ...
    def send_message(self, user_message: str) -> str:
        ...
        if self._is_open_excel_request(user_message) and self.last_excel_filename:
            ...
            latest_text = self._strip_non_english_lines(
                f"I opened {self.last_excel_filename} in Excel.\n{result}"
            )
        ...
        for _ in range(8):
            response = self.chat.send_message(pending_message)
            ...
            if not function_calls:
                if self._is_open_excel_request(user_message) and self.last_excel_filename:
                    ...
                    latest_text = f"I opened {self.last_excel_filename} in Excel.\n{result}"
                else:
            ...
            pending_message = function_responses

        fallback = self._strip_non_english_lines(latest_text or "I could not complete the request after several tool calls.")
